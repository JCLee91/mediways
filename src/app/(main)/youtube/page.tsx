...
export default function YoutubePage() {
  const [formData, setFormData] = useState<YoutubeData>({
    topic: "",
    tone: "~해요체",
    content: "",
    ...
    <>
      ...
      <div>
        <label className="block text-sm text-white font-bold mb-3">
          영상 주제 <span className="text-red-500">*</span>
        </label>
        <textarea
          ...
          rows={3}
          placeholder="영상에서 말하고자 하는 주제를 입력해 주세요."
        />
      </div>
      ...
      <div>
        ...
        <div className="relative">
          <select
            value={formData.tone}
            onChange={(e) => setFormData({ ...formData, tone: e.target.value as YoutubeData['tone'] })}
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full bg-black border border-gray-800 rounded-xl px-4 py-3 text-white font-bold placeholder-gray-500 focus:border-gray-700 focus:outline-none resize-none text-sm"
          rows={5}
          placeholder="꼭 포함되어야 하는 시술 내용을 입력해주세요."
        />
      </div>
    </>
  );
